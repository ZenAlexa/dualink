...
const CERT_FILE_NAME: &str = "dualink.pem";
...
fn default_path() -> Result<PathBuf, VarError> {
    ...
    let default_path = {
        ...
        format!("{xdg_config_home}/dualink/")
    };
...
impl From<EmulationBackend> for input_emulation::Backend {
    fn from(backend: EmulationBackend) -> Self {
        match backend {
            ...
            #[cfg(all(target_os = "macos", feature = "macos_vhid"))]
            EmulationBackend::MacOsVHID => Self::MacOsVHID,
...
impl Display for EmulationBackend {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            #[cfg(all(target_os = "macos", feature = "macos_vhid"))]
            EmulationBackend::MacOsVHID => write!(f, "macos-vhid"),
